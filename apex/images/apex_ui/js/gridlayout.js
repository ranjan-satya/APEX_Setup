 ...
 */
 ...
 ( function ( $, util, debug, lang ) {
    ...
    var C_GLV = "a-GridLayout",
        ...
        C_DEPRECATED = "a-GridLayout--deprecated",
    ...
    // map from slot type to container class [0] and container item class [1]
    var containerTypeClassMap = {
        "region": [ C_REGION_CONT, C_REGIONS ],
        "item": [ C_ITEM_CONT, C_ITEMS ],
        "button": [ C_BUTTON_CONT, C_BUTTONS ]
    };
    ...
    function addStateClasses( classes, component, stateClassMap ) {
        ...
        for ( k in stateClassMap ) {
            if ( hasOwnProperty( stateClassMap, k ) ) {
                if ( component[k] ) {
    ...
    function getStateIcon( component, stateIconMap ) {
        ...
        for ( k in stateIconMap ) {
            if ( hasOwnProperty( stateIconMap, k ) ) {
                if ( component[k] ) {
    ...
    function classesForTypes( slot, which ) {
        let containerClass = "";
        ...
        if ( slot.hasRegionSupport ) {
            ...
            containerClass += " " + containerTypeClassMap.region[which];
        }
        ...
        if ( slot.hasItemSupport ) {
            ...
            containerClass += " " + containerTypeClassMap.item[which];
        }
        ...
        if ( slot.hasButtonSupport ) {
            ...
            containerClass += " " + containerTypeClassMap.button[which];
        }
    ...
    function getTargetClassesForContainer( container$ ) {
        var classes = "",
            slot$ = container$.closest( SEL_CONTAINER );

        if ( slot$.hasClass( C_REGION_CONT ) ) {
            classes += " " + C_REGIONS;
        }
        if ( slot$.hasClass( C_ITEM_CONT ) ) {
            classes += " "+ C_ITEMS;
        }
        if ( slot$.hasClass( C_BUTTON_CONT ) ) {
            classes += " " + C_BUTTONS;
        }
    ...
    $.widget( "apex.gridlayout", $.ui.mouse, {
        ...
        _setOption: function ( key, value ) {
            ...
            if ( key === "displayFrom" ) {
                this.refresh( false );
            } else if ( [ "hideLegacySlots", "hideButtons", "hideItems", "hideEmptySlots" ].includes( key ) ) {
                this._showOrHideSlots();
            } else if ( key === "zoom" ) {
                this._setZoom();
            }
        },
        ...
        refresh: function ( focus ) {
            ...
            if ( o.displayFrom ) {
                ...
                if ( !region ) {
                    throw new Error ( "No such region in displayFrom option: " + o.displayFrom );
                }
                gridSupport = true;
                p = region._parent;
                while ( p ) {
            ...
            if ( focus ) {
                ...
                if ( sel.length > 0 && this.element.is( ":visible" ) ) {
                    ...
                    // Bug 33894807 we should only trigger a selection change when this.getSelection() actually returns
                    // data. In the case where sel.length > 0 and this.getSelection() === 0,
                    // a selectionChange is not desirable. Starting with APEX 21.2, it is possible to create a page item
                    // on a page position (example BODY), then change the item type to hidden, this triggers a gridlayout
                    // refresh, but no selectionchange should happen. The following statement ensures that doesn't happen
                    if ( this.getSelection().length > 0 && sel.length !== this.getSelection().length ) {
                        this._trigger( "selectionChange" );
                    }
                }
            }
        ...
        makeHelper: function ( type, icon, title, componentTypeId, regionType, itemType ) {
            ...
            out.markup( "<div" ).attr( "class", typeClass );
            if ( type === "region" ) {
                out.optionalAttr( "data-region-type", regionType );
            } else if ( type === "item" ) {
                out.optionalAttr( "data-item-type", itemType );
            }
            out.optionalAttr( "data-type-id", componentTypeId ).markup( ">" );
        ...
        getDropTargets: function( component$, action ) {
            ...
            if ( componentType === "item" || !componentType && component$.hasClass( C_PAGE_ITEM ) ) {
                ...
                throw new Error( "Invalid component or component type" );
            }
            targetSel = "." + targetSel;
            itemSel = "." + itemSel;
        ...
        update: function ( component, layoutChange, childrenOnly ) {
            ...
            if ( layoutChange ) {
                ...
                if ( component.type === "page" ) {
                    ...
                    gridSupport = Array.isArray( component._parent._grids );
                }
                this._layoutSlots( component.slots, component, gridSupport, 12 ); // xxx 12
            }
            ...
            if ( component$.length ) {
                if ( ! layoutChange ) {
                    ...
                    if ( component.type === "region" ) {
                        // update the region type if component is a region (this updates the slot component control)
                        component$.data( "region-type", component.regionType );
                    } else if ( component.type === "item" ) {
                        // update the item type of component is an item (this updates the slot component control)
                        component$.data( "item-type", component.itemType );
                    }

                    // update any state classes
                    component$.toggleClass( C_CONDITIONAL, !!component.isConditional );
                    for ( k in o.stateClassMap ) {
                        if ( hasOwnProperty( o.stateClassMap, k ) ) {
                    ...
                    this._showOrHideSlots( newComponent$ );
        ...
        add: function ( event, component, container$, grid, row, column, index, alignment, component$ ) {
            ...
            if ( !container$ || !container$.is( SEL_CONTAINER ) ) {
                throw new Error( "Invalid add destination" );
            }
        ...
        remove: function ( event, components$ ) {
            var i, changes, nextSelection$, elements, el$,
                changesList = [],
         ...
         */
        /*
         * Take the components of a slot and return an array of grids
        ...
        _layoutGrid: function ( components, parent, gridSupport, maxColumns ) {
            ...
            if ( gridSupport ) {
                grids = [];
                // console.log( "xxx    Pass 1: create grids", components );
                for ( ci = 0; ci < components.length; ci++ ) {
                    ...
                    if ( gridSupport && comp.col > 0 ) { // xxx and ! hidden
                        ...
                        if ( !hasOwnProperty( fixedColumns, comp.col ) ) {
                            ...
                            if ( comp.col < maxFixedCol ) {
                                ...
                                for ( j = comp.col + 1; j <= maxFixedCol; j++ ) {
                                    if ( hasOwnProperty( fixedColumns, j ) ) {
            ...
            for ( ci = 0; ci < components.length; ci++ ) {
                ...
                if ( comp.slots ) {
                    ...
                    this._layoutSlots( comp.slots, comp, gridSupport, maxColumns );
                }
            }
        ...
        _layoutSlots: function ( slots, parent, gridSupport, maxColumns ) {
            var pageInfo = this.pageRoot,
                usedColumns = 0;

            // console.log( "xxx layout slots " + parent.title + ", " + maxColumns + ", " + gridSupport );
            for ( const slot of slots ) {
                slot._parent = parent;

                // Having span or newColumn properties means that the slots will
                // be arranged in a template grid (a simpler read only grid layout)
                if ( hasOwnProperty( slot, "span" ) || hasOwnProperty( slot, "newColumn" ) ) {
                    // a few consistency checks
                    if ( slot.newGrid ) {
                        delete slot.newRow; // new grid implies a new row
                        delete slot.newColumn; // and new column
                    }
                    if ( slot.newRow ) {
                        delete slot.newColumn; // new row implies a new new column
                    }
                    if ( slot.newRow || slot.newGrid ) {
                        usedColumns = 0;
                    }
                    // keep in mind that span affects how the template grid is laid out and maxColumns
                    // applies is the number of columns the contents of the slot have available
                    slot.span = slot.span || 1;
                    if ( slot.newColumn !== false ) {
                        usedColumns += slot.span;
                    }
                    if ( pageInfo.gridType === "fixed" && usedColumns > maxColumns ) {
                        throw new Error( "Fixed layout template grid exceeded max columns. Slot: " + slot.title );
                    }
                    if ( gridSupport && slot.isGrid && !slot.maxColumns ) {
                        if ( pageInfo.gridType === "fixed" && !pageInfo.alwaysUseMaxColumns ) {
                            slot.maxColumns = slot.span;
                        } else {
                            slot.maxColumns = maxColumns;
                        }
                    }
                    if ( slot.isGrid && pageInfo.gridType === "fixed" && !pageInfo.alwaysUseMaxColumns && slot.span !== slot.maxColumns ) {
                        throw new Error( "Fixed layout span and maxColumns must be the same. Slot: " + slot.title );
                    }
                } else {
                    usedColumns = 0;
                    if ( gridSupport && slot.isGrid && !slot.maxColumns ) {
                ...
                if ( slot.hasRowSpan && !pageInfo.hasRowSpan ) {
                    throw new Error( "Row span not allowed for slot when page doesn't support row span. Slot: " + slot.title );
                }

                // console.log( "xxx layout slot type: ", slot );
                slot._grids = this._layoutGrid( slot.components, slot, (gridSupport && slot.isGrid)||slot.hasSubRegionTemplate, slot.maxColumns || maxColumns );
        ...
        _layoutPage: function () {
            ...
            this._layoutSlots( page.slots, page, true, page.maxColumns );
        },

        _showOrHideSlots: function ( el$ ) {
            var show,
                o = this.options,
                self = this;

            el$ = el$ || this.element;
            // all the legacy region slots
            el$.find( SEL_REGION_CONT ).each( function() {
                var container = self._getNode( this );

                if ( !o.hideEmptySlots && $( this ).is( SEL_LEGACY ) ) {
                    // only hide if the slot is empty
                    show = !o.hideLegacySlots || container.components.length !== 0;
                } else {
                    // only hide if hide empty and the slot is empty
                    show = !o.hideEmptySlots || container.components.length !== 0;
                }

                // Override deprecated slots
                if ( container.isDeprecated && container.components.length === 0 ) {
                    show = false;
                }
            ...
            el$.find( SEL_BUTTON_CONT ).each( function() {
                ...
                show = !( o.hideButtons || ( o.hideEmptySlots && container.components.length === 0 ) );
            ...
            el$.find( SEL_ITEM_CONT ).each( function() {
                ...
                show = !( o.hideItems || ( o.hideEmptySlots && container.components.length === 0 ) );
                if ( show && container.isLegacy ) {
                    show = !o.hideLegacySlots || container.components.length !== 0;
                }
        ...
        _find: function ( component, regionsOnly ) {
            ...
            function searchSlots( slots ) {
                let foundRegion = null;

                for ( const slot of slots ) {

                    if ( !regionsOnly || slot.hasRegionSupport ) {

                        for ( let j = 0; j < slot.components.length; j++ ) {

                            let c = slot.components[j];
                            ...
                            if ( c.slots ) {
                                foundRegion = searchSlots( c.slots );
         ...
         * - a slot object from the internal model
         * - an array containing the pair region id, slot name/id
        ...
        _setNode: function ( node ) {
            if ( node._elementId ) {
                ...
                throw new Error( "Failed to add node to nodeMap - no id" );
            }
        },
        ...
        _renderRegion: function ( out, region, gridSupport, top ) {
            ...
            out.markup( "<div" ).attr( "class", regionClass ).attr( "id", this._addNode( region ) ).attr("data-region-type", region.regionType ).markup( " tabindex='-1'>" );
            this._renderHeader( out, this.options.getIconForComponent( region ), region.title, top || region.isReadOnly ? null : false, statusIcon );
            this._renderSlots( out, region.slots, gridSupport, region.itemsAboveContent );
            out.markup( "</div>" );
        },
        ...
        _renderItem: function ( out, item ) {
            ...
            out.markup( "<div" ).attr( "class", itemClass ).attr( "id", this._addNode( item ) ).attr("data-item-type", item.itemType ).markup( " tabindex='-1'>" );
        ...
        _renderComponentsInGrid: function ( out, slot, listClass, grids ) {
            ...
            if ( grids.length > 0 ) {
                ...
                for ( g = 0; g < grids.length; g++ ) {
                    grid = grids[g];
                    out.markup( "<table role='presentation'" )
                        .attr( "class", C_GRID + " " + listClass + ( g === 0 ? " first" : "" ) )
                        .markup( ">" );
                    if ( gridType === "fixed" ) {
                        fixedHeader();
                    }
                ...
                out.markup( "<table role='presentation'" )
                    .attr( "class", C_GRID + " first " + listClass )
                    .markup( ">" );
                if ( gridType === "fixed" ) {
                    fixedHeader();
                }
        ...
        _renderComponents: function ( out, layout, components, alignment, gridSupport = false ) {
            var i, comp;
            ...
            if ( components && components.length > 0 ) {
                for ( i = 0; i < components.length; i++ ) {
                    ...
                    if ( !alignment || comp.alignment === alignment ) {
                        this._renderComponent( out, comp, gridSupport );
                    }
                }
            }
            out.markup( "</div>" );
        ...
        _renderSlots: function ( out, slots, gridSupport, itemsAboveContent ) {
            var containerClass, listClass, contentTitle,
                self = this,
             ...
             * slot with id BODY and that if there is a RIGHT_OF_IR_SEARCH_BAR button container
            ...
            function renderRegionContent() {
                ...
                if ( irSearchBarBtnDp ) {
                    out.markup( "<div" ).attr( "class", containerTypeClassMap.button[0] ).attr( "id", self._addNode( irSearchBarBtnDp ) ).markup( ">" );
                    out.markup( "<h3>" ).content( irSearchBarBtnDp.title ).markup( "</h3>" );
                    self._renderComponents( out, C_FLOW + " " + containerTypeClassMap.button[1], irSearchBarBtnDp.components );
                    out.markup( "</div>" );
                }
                out.markup( "</div>" );
            }
            ...
            for ( const slot of slots ) {
                // first see if the slot is in a grid within the template
                if ( hasOwnProperty( slot, "span" ) || hasOwnProperty( slot, "newColumn" ) ) {
                    // close previous column if needed
                    if ( colOpen && slot.newColumn !== false ) {
                        out.markup( "</td>" );
                        colOpen = false;
                    }
                    if ( !gridOpen ) {
                        out.markup( "<table role='presentation'" )
                            .attr( "class", C_TEMPLATE_GRID )
                            .markup( "><tbody><tr>" );
                        gridOpen = true;
                        slot.newColumn = true; // force creating the first column
                    } else {
                        if ( slot.newGrid ) {
                            // close prev grid and start a new one
                            out.markup( "</tr></tbody></table><table role='presentation'" )
                                .attr( "class", C_TEMPLATE_GRID )
                                .markup( "><tbody><tr>" );
                        }
                    }
                    ...
                    if ( slot.newColumn !== false ) {
                        out.markup( "<td colspan='" + slot.span + "'>" );
                        colOpen = true;
                    }
                } else {
                    if ( gridOpen ) {
                ...
                if (  [ slot.hasRegionSupport, slot.hasItemSupport, slot.hasButtonSupport ].filter( Boolean ).length > 1 || slot.hasRegionSupport || slot.hasItemSupport ) {
                    if ( btnGrpOpen ) {
                        out.markup( "</div>" );
                        btnGrpOpen = false;
                    }
                } else if ( slot.hasButtonSupport ) {
                    if ( slot.placeholder === "RIGHT_OF_IR_SEARCH_BAR" ) {
                        irSearchBarBtnDp = slot;
                        continue;
                    } else {
                        if ( btnGrpOpen ) {
                            if ( slot.hasAlignment ) {
                                out.markup( "</div>" );
                                btnGrpOpen = false;
                            }
                        } else {
                ...
                if ( slot.type === "region" && slot.hasItemSupport && slot.placeholder === "BODY" && !itemsAboveContent ) {
                    renderRegionContent();
                }

                containerClass = classesForTypes( slot, 0 );
                if ( slot.isLegacy ) {
                    containerClass += " " + C_LEGACY;
                }
                if ( slot.isDeprecated ) {
                    containerClass += " " + C_DEPRECATED;
                }
                listClass = classesForTypes( slot, 1 );
                out.markup( "<div" )
                    .attr( "class", containerClass )
                    .attr( "id", this._addNode( slot ) )
                    .attr( "data-position-type", slot.type )
                    .attr( "data-position", slot.placeholder )
                    .optionalAttr( "data-supported-region-types", slot.supportedRegionTypes?.join( "," ) )
                    .optionalAttr( "data-supported-item-types", slot.supportedItemTypes?.join( "," ) )
                    .markup( ">" );
                // xxx need to also keep track of remaining columns under some cases
                out.markup( "<h3>" )
                    .content( formatPostfix( "MODEL.POSTFIX.LEGACY", slot.title, slot.isLegacy ) )
                    .markup( "</h3>" );

                if ( [ slot.hasRegionSupport, slot.hasItemSupport, slot.hasButtonSupport ].filter( Boolean ).length > 1 || slot.hasRegionSupport || slot.hasItemSupport ) {
                    if ( slot.isGrid && gridSupport ) {
                        this._renderComponentsInGrid( out, slot, listClass, slot._grids );
                    } else {
                        this._renderComponents( out, C_STACK + " " + listClass, slot.components, null, slot.hasSubRegionTemplate );
                    }
                } else if ( slot.hasButtonSupport ) {

                    if ( slot.hasAlignment ) {
                        out.markup( "<table role='presentation'" )
                            .attr( "class", C_BTN_ROW )
                            .markup( "><tbody><tr><td>" );
                        this._renderComponents( out, C_FLOW + " " + listClass, slot.components, "left" );
                        out.markup( "</td><td>" );
                        this._renderComponents( out, C_FLOW + " " + listClass, slot.components, "right" );
                        out.markup( "</td></tr></tbody></table>" );
                    } else {
                        this._renderComponents( out, C_FLOW + " " + listClass, slot.components );
                    }
                } else {
                    throw new Error( "Unknown slot type: " + slot.placeholder );
                }
                out.markup( "</div>" );
        ...
        _renderPage: function ( out, page ) {
            ...
            this._renderSlots( out, page.slots, true );
            out.markup( "</div>" );
        },
        ...
        _traverseDown: function ( event ) {
            ...
            if ( event.which !== $.ui.keyCode.RIGHT ) {
                ...
                // If button group not found, lets just continue using the previous target we had
            }
                ...
                if ( next$.length > 0 ) {
                    ...
                    if ( next$.length > 0 ) {
                        ...
                        while ( target$.length > 0 ) {
                            ...
                            // Switch closest group, handle special C_TEMPLATE_GRID
                            target$ = target$.parent().closest( "." + C_TEMPLATE_GRID + "," + SEL_COMPONENT + "," + SEL_CONTAINER + "," + SEL_BUTTON_GROUP );
                        }
                    }
                }
            }
            ...
            if ( prev$.length > 0 ) {
                ...
                if ( prev$.length > 0 ) {
                    ...
                    if ( prev$.length === 0 ) {
                        ...
                        // Special case to double check if new target is the whole page. If so, maybe we have a template grid above not being found.
                        if ( target$.is('.' + C_PAGE) ) {
                            // Check if in grid container
                            container$ = container$.parent().closest( '.' + C_TEMPLATE_GRID );
                            prev$ = container$.prevAll( ":visible" ).find( SEL_COMPONENT ).filter( ":visible" ).last();
                            if (prev$.length !== 0){
                                target$ = prev$;
                            }
        ...
        _mouseDrag: function ( event ) {
            var i, j, container, item, itemElement, intersection,
...
//      console.log("xxx mouse drag after scroll top " + this.position.top + ", pagey " + event.pageY);
        ...
        _initDragTargets: function( item$ ) {
            // Make sure to reset the itemType and regionType of this,
            // otherwise it will still have the itemType or regionType of the previous dragged item$
            this.itemType = undefined;
            this.regionType = undefined;
            if ( item$.hasClass( C_PAGE_ITEM ) ) {
                this.targetClass = C_ITEMS;
                this.itemClass = C_PAGE_ITEM;
                this.itemType = item$.data("item-type");
            } else if ( item$.hasClass( C_REGION ) ) {
                this.targetClass = C_REGIONS;
                this.itemClass = C_REGION;
                this.regionType = item$.data("region-type");
        ...
        _initTargetContainers: function () {
            ...
            if ( this.helper ) {
                // Dragging a component from the gallery view to the glv, we have a helper
                targets = targets.not( this.helper.find( targetSel ) );
                // If region type is not set, get it from helper
                regionType = regionType || this.helper.data("region-type");
                // If item type is not set, get it from helper
                itemType = itemType || this.helper.data("item-type");
            }
            ...
            // Filter target slots by those that support the itemType
            if ( itemType ) {
                targets = targets.filter( function () {
                    // Filter by itemType
                    const itemTargetContainer = $( this ).closest( SEL_ITEM_CONT );
                    const supportedItemTypes = itemTargetContainer.data( "supported-item-types" );
                    // If this target container (slot) has defined supported item types, filter on current dragged
                    // component's item type. Otherwise, don't filter.
                    return !supportedItemTypes || supportedItemTypes.includes( itemType );
                } );
            }

            // Filter target slots by those that support the regionType
            if ( regionType ) {
                targets = targets.filter( function () {
                    // Filter by regionType
                    const regionTargetContainer = $( this ).closest( SEL_REGION_CONT );
                    const supportedRegionTypes = regionTargetContainer.data( "supported-region-types" );
                    // If this target container (slot) has defined supported region types, filter on current dragged
                    // component's region type. Otherwise, don't filter.
                    return !supportedRegionTypes || supportedRegionTypes.includes( regionType );
                } );
            }
...
//xxx        helper.height(this.dragComponent.height());
        ...
        _movePlaceholder: function ( container, item, place, rowIndex, colIndex ) {
            var el, rows$, row$, cols$, count, itemSel, containerClasses,
                tempRow$ = null,
                tempCol$ = null,
                moved = false;
...
//                    console.log( "xxx move placeholder to empty stack or flow container" );
                ...
                if ( Object.prototype.hasOwnProperty.call( obj, name ) ) {
                    ...
                    if ( name === "slots" ) {
                        ...
                        for ( const slot of obj.slots ) {
                            a.push( this._copyComponent( slot, newObj ) );
                        }
        ...
        _insert: function ( changes, newGridSupport, grid, row, column, index, alignment ) {
            ...
            // Update grid if there is grid support, assuming this component
            // is not a region with sub region template in which case there is no grid
            // Reference bug 20351490 and 34119174
            if ( newGridSupport && !newParentContainer.hasSubRegionTemplate ) {
                // update grid
        ...
        _insertInDom: function ( component$, container$, newParentContainer, grid, row, column, index, isNew, alignment ) {
            ...
            if ( this.placeholder ) {
                ...
                if ( newParent$.is( SEL_GRID ) ) {
                    ...
                    if ( index === NEW_GRID && newParentContainer._grids.length > 1 ) {
                        grid$ = $( "<table role='presentation'><tbody></tbody></table>" );
            ...
            if ( isNew ) {
                this._showOrHideSlots( component$ );
            }
        },
        ...
        _add: function ( component, container$, grid, row, column, index, alignment, component$ ) {
            ...
            gridSupport = Array.isArray( newParentContainer._grids );

            if ( component.slots ) {
                maxColumns = 12; // xxx
                this._layoutSlots( component.slots, component, gridSupport, maxColumns );
            }
        ...
        _remove: function ( component$, changes, prevParent$ ) {
            ...
            if ( prevIsGrid ) {
                ...
                prevGrid = indexOf( pc._parentRow._parentGrid._parent._grids, pc._parentRow._parentGrid );
        ...
        _moveOrCopy: function ( component$, container$, grid, row, column, index, alignment, copy ) {
            ...
            component = this._getNode( component$[0] );
            changes.newParentContainer = newParentContainer = this._getNode( container$[0] );
            newGridSupport = Array.isArray( newParentContainer._grids );
            changes.prevContainer$ = component$.closest( SEL_CONTAINER );
            ...
            if ( copy ) {
                ...
                maxColumns = 12; //xxx
                if ( component.slots ) {
                    this._layoutSlots( component.slots, component, newGridSupport, maxColumns );
                }
                ...
                if ( component.slots && newGridSupport !== prevGridSupport ) {
                    maxColumns = 12; //xxx
                    this._layoutSlots( component.slots, component, newGridSupport, maxColumns );
                    this._renderComponent( out, component, newGridSupport );
                    changes.newComponent$ = $( out.toString() ); // can't use component$.clone() because changes are possible
...
//                console.log("xxx selection after drop " + newComponent$.length);
    ...
    $.ui.plugin.add( "draggable", "connectToGridlayout", {
        ...
        stop: function ( event, ui ) {
            ...
            inst.grids.forEach( function ( el ) {
                if ( el.instance.isOver && !el.invalid ) {

                    el.instance.isOver = false;
                    ...
                    if ( !event.target ) {
                        ...
                        el.instance._mouseStop( event, true );
                        // remove the temp drag component before cancel
                        el.instance.dragComponent.remove();
                        el.instance._cancel( event );
                    } else {
                    ...
                    el.instance._deactivate();
                    uiObj = $.extend( {}, ui, { item: inst.element } );
                    el.instance._trigger( "deactivate", event, uiObj );
                }
        ...
        drag: function ( event, ui ) {
            ...
            for ( let el of inst.grids ) {
                ...
                el.instance.positionAbs = inst.positionAbs;
                el.instance.helperProportions = inst.helperProportions;
                el.instance.offset.click = inst.offset.click;

                if ( el.instance._intersectsWith( el.instance.containerCache ) ) {
                    intersecting = true;
                }

                if ( intersecting ) {
                    // If it intersects, we use a little isOver variable and set it once, so our move-in stuff gets fired only once
                    if ( !el.instance.isOver ) {

                        el.instance.isOver = true;
                        ...
                        if ( !el.instance._makeTempDragComponent() ) {
                        ...
                        el.instance.helper = ui.helper;
                        el.instance.helper.css( "position", "relative" ); // for proper scrollParent detection, it will get put back to absolute by _mouseStart

                        event.target = el.instance.dragComponent.children( "h3" )[0];
                        if ( !el.instance._mouseCapture( event, true ) ) {
                            el.instance.isOver = false;
                            el.invalid = true;
                            continue;
                        }

                        el.instance._trigger( "over", event, el.instance._uiHashDnD( el.instance ) );
                        el.instance._mouseStart( event, event, true );

                        // Because the browser event is way off the temp drag component, we modify a couple of variables to reflect the changes
                        el.instance.offset.click.top = inst.offset.click.top;
                        el.instance.offset.click.left = inst.offset.click.left;
                        el.instance.offset.parent.left -= inst.offset.parent.left - el.instance.offset.parent.left;
                        el.instance.offset.parent.top -= inst.offset.parent.top - el.instance.offset.parent.top;

                        inst.dropped = el.instance.element; //draggable revert needs that
                        //hack so receive/update callbacks work (mostly)
                        inst.currentItem = inst.element;
                        el.instance.fromOutside = inst;

                    }

                    // Provided we did all the previous steps, we can fire the drag event of the gridlayout on every draggable drag
                    if ( el.instance.dragComponent ) {
                        el.instance._mouseDrag( event );
                    }
                    ...
                    if ( el.instance.isOver ) {
                        ...
                        el.instance._trigger( "out", event, el.instance._uiHashDnD( el.instance ) );
                        ...
                        el.instance.dragComponent.remove();
                        if ( el.instance.placeholder ) {
                            el.instance.placeholder.remove();
                        }
