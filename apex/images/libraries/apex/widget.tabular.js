...
*/
/*
 * The tabular namespace is used to store functions specific to tabular forms in Oracle APEX.
 */
/**
...
( function( tabular, report, $, undefined ) {
    ...
    tabular.init = function ( pRegionId, pAjaxIdentifier, pOptions ) {
        ...
        // Bind click handler for check-all-rows checkbox
        $( "#check-all-rows" ).on( "click", ( event ) => {
            tabular.checkAll( event.target );
        } );
     ...
     * @memberOf apex.widget.tabular
     */
    tabular.paginate = function ( pRegionId, pAjaxIdentifier, pPaginationData ) {

        function _paginate() {
            if ( tabular.gAjaxEnabled ) {
                report.paginate( tabular.gInternalRegionId, pAjaxIdentifier, pPaginationData );
            } else {
     ...
     * @memberOf apex.widget.tabular
     */
    tabular.sort = function ( pRegionId, pAjaxIdentifier, pSortData, pThis ) {

        function _sort() {
            if ( tabular.gAjaxEnabled ) {
                report.sort( pThis, pAjaxIdentifier, pSortData );
            } else {
    ...
    tabular.addRow = function (pNewRowVals, pNewRowMap, pDispTypeMap) {
        ...
        $( ":input[id^=f][id$=_" + lNewRowIndex + "]", apex.gPageContext$ ).on( "change", function() {
