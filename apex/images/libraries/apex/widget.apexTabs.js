...
/**
 * @fileOverview
 * The {@link apex.widget}.apexTabs is used for tab or tab-like widgets in Oracle APEX.
 *
 **/

(function( $, lang, server, debug, widgetUtil ) {
    "use strict";
    ...
    $.widget("apex.aTabs", {
        options: {
            ...
            allowSwipe: false,
            ajaxIdentifier: null,     // the identifier used when making an AJAX call to the region
            storedTabPreference: null // the server-side tab preference stored for Region Display Selectors
        },
        ...
        _onClick: function ( tab, e ) {
            ...
            if (e === undefined || !e.doNotFocus ) {
                tab.link$.trigger( "focus" );
            }
            // store id of clicked tab if we are supposed to remember it
            if ( this.options.useStoredPreference ) {
                this._setPreference( tab.href.slice(1) );
            }
        },

        // Make AJAX request to store the id of the selected tab (pTabId) as the new active tab preference
        _setPreference: function( pTabId ) {
            let lData = {
                    x01: pTabId
                },
                lOptions = {
                    ...
                    error: function( jqXHR, textStatus, errorThrown ) {
                        debug.error( "Error during apexTabs._setPreference Ajax call.", jqXHR, textStatus, errorThrown );
                    }
                };
        ...
        _hideTabInList: function ( tab ) {
            if (this.activeTab === tab) {
                // bug 35711528: check that previous/next tab isn't
                // hidden, we don't want to make a hidden tab active
                if (tab.previous && !tab.previous.hidden) {
                    tab.previous.makeActive(true);
                } else if (tab.next && !tab.next.hidden) {
                    tab.next.makeActive(true);
                } else { // make the next visible tab active
                    tab.getNextVisible()?.makeActive(true);
                }
            }
            tab.hidden = true;
        ...
        _iterateThroughAndClear: function (tab, pMoveToPrevious, pShowAll) {
            ...
            while (tab !== null) {
                ...
                // We only set aria-controls on currently selected tab now when not pShowAll. bug #33592031
                if (pShowAll) {
                    tab.link$.attr( A_CONTROLS, tab.href.substring(1) );
                } else {
                    tab.link$.removeAttr( A_CONTROLS );
                }
                if (pMoveToPrevious) {
        ...
        _create: function () {
            ...
            var options = this.options,
                ...
                pUseStoredPreference = options.useStoredPreference,
                pStoredTabPreference = options.storedTabPreference;
            ...
            var overflowIsAuto = function () {
                return tabs$.parent().css("overflow") === "auto";
            };
            ...
            var initializeSlider = function () {
                ...
                if (inFillMode() || overflowIsAuto() || !pUseSlider || slider$ !== null) {
                    return;
                }
...
//            tabsContainer$.scroll(checkState);
            ...
            var buildTabs = function () {
                ...
                if (options.addMoveNextPrevious) {
                    ...
                    tabs$.parent()
                        ...
                        .append(moveNext$);
                    movePrevious$.on( EVENT_CLICK, function () {
                        me.movePreviousActive();
                    });
                    moveNext$.on( EVENT_CLICK, function () {
                        me.moveNextActive();
                    });
                }
                if ( jumpMode ) {
                ...
                if (links$.length <= 2 && links$.eq(0).attr("href") === SHOW_ALL) { // Don't initialize the RDS if this is a legacy RDS with only two tabs!
                    // Also remove class used to hide regions while getting activeTab when there's more than 2 tabs
                    $('.apex-rds-region').removeClass( 'apex-rds-region' );
                    tabs$.remove();
                    return;
                }
                ...
                links$.each(function () {
                    ...
                    var scrollToTab = function (pTab) {
                        if (inFillMode() || !pUseSlider || overflowIsAuto()) {
                            return;
                        }
                    ...
                    var forceActive = function ( scrollToActive ) {
                        ...
                        if ( href !== SHOW_ALL ) {
                            this.link$.attr( A_CONTROLS, href.substring(1) );
                        }
                        ...
                        // todo this widget MUST NOT have knowledge of other widgets. In each widget should use widgetUtil.onVisibilityChange
                    ...
                    link$.on( EVENT_CLICK, function (e) {
                    ...
                    link$.on( "keydown" , function (event) {
                        ...
                        if (kc === keys.UP) {
                            ...
                            me.firstVisibleTab.link$.trigger( EVENT_CLICK );
                        } else if (kc === keys.END) {
                            me.lastVisibleTab.link$.trigger( EVENT_CLICK );
                        } else if (kc === keys.SPACE) {
                            currentTab.link$.trigger( EVENT_CLICK );
                        } else {
                            return;
                        }
                    ...
                    realNumberOfTabs = realNumberOfTabs + 1;
                });
                ...
                if (me.activeTab === null) {
                    ...
                    if (pUseSessionStorage || pUseStoredPreference) {
                        var href = pStoredTabPreference ? pStoredTabPreference : sessionStorage.getItem( regionId + ".activeTab" );
                        for (var key in tabs) {
                            if ( hasOwnProperty( tabs, key ) ) {
                ...
                tabs$.on( EVENT_CLICK, function () {
                    me.firstVisibleTab.link$.trigger( "focus" );
                });
            ...
            var initializeStandardTabs = function () {
                ...
                if (me.activeTab) {
                    if (me.activeTab.href !== SHOW_ALL) {
                        ...
                        // forceActive the SHOW_ALL active tab on load as well
                        // this allows consistent functionality for all tabs on load, and adds the aria attributes that come with it
            ...
            if ( options.allowSwipe && ( options.tabsContainer$ instanceof $ ) ) {
                options.tabsContainer$.on('apexswipe', function ( event, hammerExtras ) {
                    ...
                    switch ( direction ) {
                        case 2: // swipe direction is to the left, so make next carousel item active
                            me.moveNextActive();
                            break;
                        case 4: // swipe direction is to the right, so make previous carousel item active
                            me.movePreviousActive();
                            break;
                    }
                });
    ...
    $.apex.aTabs.findClosestTab = function( pElement ) {
        let panel$, aTabs$, tabId,
            tab = null;

        panel$ = $( pElement ).parents( "." + C_TAB_PANEL );
        if ( panel$.length ) {
            tabId = "#" + panel$[0].id;
    ...
    if ( widgetUtil.visibilityChange ) {
        $( document.body ).on( "atabsactivate", function ( e, ui ) {
            ...
            if ( ui.active.href === SHOW_ALL ) {
                ...
                for ( t in allTabs ) {
                    if ( hasOwnProperty( allTabs, t ) ) {
                        if ( ui.previous && t !== ui.previous.href && t !== SHOW_ALL ) {
                            widgetUtil.visibilityChange( allTabs[t].panel$[0], true );
                        }
                    }
                }
                ...
                for ( t in allTabs ) {
                    if ( hasOwnProperty( allTabs, t ) ) {
                        if ( t !== ui.active.href && t !== SHOW_ALL ) {
                            widgetUtil.visibilityChange( allTabs[t].panel$[0], false );
                        }
                    }
                }
            } else {
                if ( oldTab ) {
                    widgetUtil.visibilityChange( oldTab, false );
                }
                widgetUtil.visibilityChange( newTab, true );
            }
        });
    }
    ...
    apex.widget.regionDisplaySelector = function (pRegionDisplaySelectorRegion, pOptions) {
        ...
        // enable Region Display Selector tab selection (bug #34371688)
        rds$.removeClass( "is-disabled" );

        // Bug #34371688
        // It's possible to have regions that are part of a Region Display Selector and that also have
        // Server-side Conditions using bind variables which haven't been evaluated yet. In that case,
        // the number of tabs and regions are inequal and we should remove the CSS added to RDS regions.
        const rdsTabs    = $( ".apex-rds-item:not(#SHOW_ALL_tab)" ),
              rdsRegions = $( ".apex-rds-region" );
        if ( rdsTabs.length !== rdsRegions.length ) {
            rdsRegions.each( function() {
                this.classList.remove('apex-rds-region');
            } );
        }
        // In the builder, the code in wwv_flow_javascript / standard_javascript gets added at the top
        // of the page instead of the bottom like it is for runtime apps. This means the active Region
        // Display Selector region(s) won't have the selected class added yet so we need to add it here.
        if ( $( ".apex-rds-element-selected" ).length === 0 ) {
            const activeTabId = sessionStorage.getItem( 'ORA_WWV_apex.apexTabs.'      +
                                                         apex.env.APP_ID      + '.'   +
                                                         apex.env.APP_PAGE_ID + '.R'  +
                                                         pRegionDisplaySelectorRegion + '.activeTab' );
            $( document ).find( activeTabId ).addClass( "apex-rds-element-selected" );
        }
...
})( apex.jQuery, apex.lang, apex.server, apex.debug, apex.widget.util );
