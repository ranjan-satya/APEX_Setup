...
function useTabbableModeSet(rootRef, keyExtractor, currentKey, onCurrentKeyChange, keySupportsTabbable, keyComparator) {
    ...
    const disableTabbableMode = hooks.useCallback((target, isKeyboardEvent = false) => {
        ...
        if (collectionUtils.getElementContainsFunc(rootRef.current, true)(target) && isKeyboardEvent) {
    ...
    const getCurrentTabbableKey = hooks.useCallback(() => {
    ...
    const onFocus = (event) => {
        ...
        if (event.target === rootRef.current) {
            ...
            if (keys.isKeyDefined(key) && !isCurrentTabbableKey(key)) {
    ...
    const onBlur = (event) => {
        if (!event.relatedTarget ||
            !collectionUtils.getElementContainsFunc(rootRef.current, true)(event.relatedTarget)) {
...
//# sourceMappingURL=useTabbableModeSet-bfbdbba0.js.map
