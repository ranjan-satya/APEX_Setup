...
var UNSAFE_TextField_themes_ObfuscatedTextFieldInputStyles_css = require('./UNSAFE_TextField/themes/ObfuscatedTextFieldInputStyles.css.js');
...
function useHistoryManager({ state, comparator, isDisabled, maxHistory }) {
    ...
    const initialMaxHistoryRef = compat.useRef(maxHistory);
    const historyManager = compat.useMemo(() => new HistoryManager({ maxHistory: initialMaxHistoryRef.current }), []);
    ...
    const isIgnoredRef = compat.useRef(false);
    ...
    const previousStateRef = compat.useRef();
    ...
    compat.useEffect(() => () => historyManager.clear(), [historyManager]);
...
const ObfuscatedTextFieldInput = ({ character = '•', currentCommitValue, inputRef, isRevealed = false, onCommit, onInput, type = 'text', value, ...passThroughProps }) => {
    ...
    // again.
    const selectionRangeRef = compat.useRef({
        selectionStart: value?.length ?? 0,
        selectionEnd: value?.length ?? 0
    });
    const internalInputRef = compat.useRef(null);
    const mergedInputRef = compat.useMemo(() => refUtils.mergeRefs(inputRef, internalInputRef), [inputRef, internalInputRef]);
    ...
    const onInputPayloadRef = compat.useRef();
    /**
     ...
     */
    const STABLE_restoreSelectionRange = useEffectEvent.useEffectEvent((_) => {
    ...
    const computeNextInput = compat.useCallback((event) => {
    ...
    const triggerOnInputCallback = compat.useCallback(() => {
        // if the component is revealing the current value, then let it
    ...
    const selectAllWhenObfuscated = compat.useCallback(() => {
    ...
    const killEventWhenObfuscated = compat.useCallback((event) => {
        if (isRevealed)
            return;
    ...
    const onInputHandler = compat.useCallback((details) => {
    ...
    const onCommitHandler = compat.useCallback(() => {
    ...
    const onKeyDownHandler = compat.useCallback((event) => {
    ...
    return (jsxRuntime.jsx("div", { class: UNSAFE_TextField_themes_ObfuscatedTextFieldInputStyles_css.containerStyles, 
        ...
        onContextMenu: selectAllWhenObfuscated, children: jsxRuntime.jsx(TextFieldInput.TextFieldInput, { ...passThroughProps, 
            // override properties
...
exports.ObfuscatedTextFieldInput = ObfuscatedTextFieldInput;
//# sourceMappingURL=ObfuscatedTextFieldInput-c6e6e803.js.map
