        ...
        floatingLabels: function() {
            ...
            var needLabel = function ( item$ ) {
                var placeholder = item$.attr( 'placeholder' ) || '',
                    firstOptionText = item$.children( 'option' ).first().text() || '';

                return !apex.item(item$.attr('id')).isEmpty() ||
                    placeholder.length ||
                    firstOptionText.length;
            };
            ...
            var popupElementBlur = function( event ) {
                triggerBlur(lastItem);
            };

            var popupElementFocused = function( event ) {
                // if the focus goes to a popup, we clear the blur timer
            ...
            $( tags )
                ...
                .on( 'change', function ( e ) {
                    ...
                    // check if the label needs to float
                    if ( needLabel( item$ ) ){
                        container$.addClass( CL_SHOW_LABEL );
                    } else {
                        container$.removeClass( CL_SHOW_LABEL );
                ...
                .on( 'focus', function ( e ) {
                    ...
                    // if there's a blur timer and the focus goes back to the same item
                    // we need to remove the delayed blur timer
                    if ( blurDelayTimer && lastItem === this) {
                        clearBlurTimer();
                    }

                    // check if the item has a popup
